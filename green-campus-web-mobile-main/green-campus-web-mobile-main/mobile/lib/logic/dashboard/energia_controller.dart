...
class EnergiaController extends ChangeNotifier {
  ...
  void init(List<IotNode> allNodes) {
    ...
    _nodes = allNodes.where((n) {
      ...
      return search.contains('energ') || search.contains('potenc') || search.contains('cons');
    }).toList();
  ...
  Future<void> _loadData() async {
    try {
      ...
      debugPrint("Error cargando datos de energía: $e");
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  ...
  void _calculateAverage() {
    ...
    _averageEnergy = sum / values.length;
  }
  ...
  Future<void> _fetchLastReading(String nodeId, DateTime now) async {
    try {
      ...
      // Extraer el valor de energía
      final raw =
          item['energy']?.toString() ?? 
          item['consumo_w']?.toString() ?? 
          item['value']?.toString() ?? '';
  ...
  double? energyOf(IotNode node) => _nodeValues[node.id];
