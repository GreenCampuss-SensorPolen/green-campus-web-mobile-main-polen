...
class HumedadController extends ChangeNotifier {
  ...
  void init(List<IotNode> allNodes) {
    ...
    _nodes = allNodes.where((n) {
      ...
      return name.contains('hum') || type.contains('hum');
    }).toList();
  ...
  Future<void> _loadData() async {
    try {
      ...
      debugPrint("Error cargando datos de humedad: $e");
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  ...
  void _calculateAverage() {
    ...
    if (values.isEmpty) {
      _averageHumidity = null;
      return;
    }
    final sum = values.fold(0.0, (a, b) => a + b);
    _averageHumidity = sum / values.length;
  }
  ...
  double? humidityOf(IotNode node) => _nodeValues[node.id];
